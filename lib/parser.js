...
export default {
     ...
     */
    toHtml: function (tree, options = { singleTags: [], closingSingleTag: 'default'}) {
         ...
         *
         */
        let { singleTags: optSingleTags, closingSingleTag } = options;

        const SINGLE_TAGS = ['area', 'base', 'br', 'col', 'command', 'embed', 'hr', 'img', 'input', 'keygen',
                            'link', 'menuitem', 'meta', 'param', 'source', 'track', 'wbr'];
        let singleTags = {};

        for (let i = 0, l = SINGLE_TAGS.length; i < l; i++) {
            singleTags[SINGLE_TAGS[i]] = 1;
        }

        if (optSingleTags) {
            for (let i = 0, l = options.singleTags.length; i < l; i++) {
                singleTags[options.singleTags[i]] = 1;
            }
        }
