...
function lazyResult (render, tree) {
  return {
    ...
    template: function (locals) {
      return template(render(tree, tree.options), locals)
    },
 ...
 *
 * posthtml(plugins).process(html, options)
 *   .then((result) => {
 *     return result.template({ hello: 'Hello World!' })
 *   })
 *   .then((html) => {
 *      //<div>Hello World!</div>
 *      console.log(html)
